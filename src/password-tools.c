...
int check_password_strength(char *pass)
{
    ...
    if (strength_level >= 4)
    {
        printf("\033[1;32mPassword is strong\033[0m\n"); // Green
        return 4;
    }
    else if (strength_level == 3)
    {
        printf("\033[1;36mPassword is just good\033[0m\n"); // Cyan
        return 3;
    }
    else if (strength_level == 2)
    {
        printf("\033[1;33mPassword is weak\033[0m\n"); // Yellow
        return 2;
    }
    else
    {
        printf("\033[1;31mPassword is too weak\033[0m\n"); // Red
        return 1;
    }
}
...
int take_password_input(char *pass, size_t pasword_limit, char *print_message, bool is_strict)
{
    ...
    while (1)
    {
        bool is_auto_gen_password = false;
        ...
        if(strlen(pass)==1 && pass[0]=='1') {
            generate_secure_password(pass,16);
            printf("Auto Generated Password : %s\n",pass);
            is_auto_gen_password = true;
        }
        if (!is_auto_gen_password){
            int strength_status = check_password_strength(pass);
            if (is_strict){
                if (strength_status < 2){
                    continue;
                }
            }
        }
...
int generate_dek(char *PASSWORD, unsigned char DEK[crypto_secretbox_KEYBYTES], bool is_update_password)
{
    ...
    if(fwrite(&vault, sizeof(vault), 1, fptr)!=1){
        fclose(fptr);
